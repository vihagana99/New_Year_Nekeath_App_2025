...
import 'package:timezone/data/latest.dart' as tz_data; // Import the timezone data
...
class NotificationService {
  ...
  static Future<void> scheduleNotification(String eventTitle, Duration duration) async {
    // Check if the duration is at least 5 minutes
    if (duration.inMinutes >= 5) {
      // Calculate the notification time
      DateTime notificationTime = DateTime.now().add(Duration(minutes: duration.inMinutes - 5));
      ...
      await _notificationsPlugin.zonedSchedule(
        0,
        "Event Reminder",
        "$eventTitle ඉතුරු වෙන්නේ මිනිත්තු 5යි!", // Message showing 5 minutes remaining
        ...
        androidAllowWhileIdle: true, // Allow notification to be shown even if the app is idle
        uiLocalNotificationDateInterpretation:
        UILocalNotificationDateInterpretation.absoluteTime, // Ensure the notification fires at the correct time
      );
    }
  }
}
